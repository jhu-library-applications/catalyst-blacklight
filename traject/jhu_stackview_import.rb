...
# database_yml_path = File.expand_path(Rails.root.join("config", "database.yml"), __FILE__)
...
each_record do |record, context|
  ...
  JhShelfBrowseExtractor.new(context).stackview_output_contexts.each do |s_context|
    ...
    context.output_hash = s_context.output_hash
  end
end
...
class JhShelfBrowseExtractor
  ...
  def identifier
    m245 = "bib_#{source_record['001'].to_s.split(' ').last}"
  end
